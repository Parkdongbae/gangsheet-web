 ...
 * 드래그 이동 → PNG 내보내기(치수·pHYs 350 DPI Node 검증) → .dtf 저장 다운로드 →
 * 재로드 후 IndexedDB 자동저장 복원. 페이지 오류 0 단정.
 *
...
async function startPreview() {
  const proc = spawn('npx', ['vite', 'preview', '--config', 'web/vite.config.ts', '--port', String(PORT), '--strictPort'], {
    cwd: ROOT,
    shell: true,
    stdio: 'ignore'
  })
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => window.__lastScene?.images?.length >= 1,
      undefined,
      { timeout: 20000 }
    )
    ...
    const bg = await page.evaluate(async (path) => {
      ...
      return { cornerAlpha: corner[3], centerAlpha: center[3], w: result.widthPx, h: result.heightPx }
    }, importResult)
    ...
    assert(meta.width === 689 && meta.height === 13780, `PNG 치수 5cm×1m @350DPI (${meta.width}×${meta.height})`)
    ...
    const project = JSON.parse(readFileSync(await saveDownload.path(), 'utf8'))
    assert(project.format === 'dtf-gangsheet-project' && project.version === 1, '.dtf 스키마 형식·버전')
    assert(project.document.widthPx === 689 && project.document.heightPx === 13780, '.dtf 문서 규격')
    ...
    assert(restoredCount === 'db-present', '자동저장 복원 — 편집 화면(이미지 있음) + IndexedDB 유지')
